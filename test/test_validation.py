...
from xlsx_value_picker.validation import ValidationContext, ValidationResult, ValidationEngine
from xlsx_value_picker.config_loader import (
    CompareExpression, RequiredExpression, RegexMatchExpression, EnumExpression,
    AllOfExpression, AnyOfExpression, NotExpression, Rule
)
...
class TestValidationContext(unittest.TestCase):
    ...
    def test_get_field_value(self):
        ...
        context = ValidationContext(
            cell_values={"name": "山田太郎", "age": 30},
            field_locations={"name": "Sheet1!A1", "age": "Sheet1!B1"}
        )
...
class TestValidationEngine(unittest.TestCase):
    ...
    @patch('xlsx_value_picker.validation.get_excel_values')
    def test_validate(self, mock_get_excel_values):
        """バリデーションエンジンのテスト"""
        # モックの設定
        mock_get_excel_values.return_value = {
            "age": 25,
            "email": "invalid-email"
        }
